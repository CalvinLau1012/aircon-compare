...
> **能源級別、雪種、年耗電以機電署 EMSD 官方資料庫全量核實**（最新同步快照 2026-09-20：1,885 筆登記／1,824 型號；以線上 [`metadata.json`](https://calvinlau1012.github.io/aircon-compare/metadata.json) 為準）
...
- 🛒 1,808 個型號附價格快照（2026-09-20 同步快照重算；BigGo 官方 API 快照 751 有價 + Price 舊快照 1,847 後備；缺價標「待查」），點擊 🔍 直接在你的瀏覽器用 Google 搜最新價
...
| 項目 | 數量 |
| ------ | ------ |
| 比較器總型號 | **1,824**（核心 29 + 其餘 EMSD 型號 1,795（全量去重為 1,824，包含核心 29）；＝ metadata.json `modelCount` · 2026-09-20 快照） |
| EMSD 官方登記 | **1,885 筆**（`registrationCount`／`rawRecordCount` · 2026-09-20 EMSD 收據；登記筆數與型號數口徑不同，不可互換） |
| 有價格型號 | 1,808（網站 hero 依同步快照重算，2026-09-20） |
| 有尺寸型號 | 1,636（同上，按快照重算） |
...
### 📈 數據統計（2026-09-20 同步快照 · 同一資料源即時計算）

#### 狀態分佈（比較器標籤 · canonical 型號鍵）

```mermaid
pie showData title 型號狀態分佈（1,824）
    "有價" : 672
    "停售（保留舊快照）" : 1076
    "官方價" : 65
    "無價" : 11
...
> 2026-09-20 快照：canonical `BRAND|NORM`（D11）匹配後，停售 1,076、有價 672、官方價 65、無價 11，合共 1,824。
...
#### 能源級別（EMSD 全量 canonical model · 2026-09-20）
...
xychart-beta
    ...
    y-axis "型號數" 0 --> 1200
    bar [1138, 166, 168, 347, 5]
```

> canonical model 按 `BRAND|NORM` 去重（1,824 個）：1 級 1,138／2 級 166／3 級 168／4 級 347／5 級 5；EMSD registration（1,885 筆登記）為 1,195／166／172／347／5。核心 29 精選為 1 級 13、3 級 9、4 級 7、2／5 級 0——三種口徑不同，不可混用。
...
| `emsd_空調能源標籤.csv` | EMSD 官方資料庫快照（2026-09-20 抓取：1,885 筆登記 / 1,824 型號；以 `metadata.json` 為準） |
