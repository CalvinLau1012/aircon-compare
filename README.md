...
- **鏈接分批驗證**：`verify_links.py` 每日分批核實 Price 產品鏈接（全量日 400 個 / 平日 200 個，約 4-5 日完成一輪全量核實）；錯配即修正，網絡失敗/限流自動保留原鏈接同熔斷中止
- **禮貌**：誠實 Bot UA 標明專案來源、退避重試遵從 Retry-After、隨機抖動延遲；價格每週全量刷新、平日只補缺（大幅降負載）；限流熔斷（連續失敗即中止，保留舊數據）
...
1. 價格及供應隨時間變動；官方網店價為 2026-08-15 快照（每日 00:30 自動更新後以最新版為準）
