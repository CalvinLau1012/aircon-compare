...
def main():
    ...
    urls = json.load(open(os.path.join(base, 'carrier_urls.json'), encoding='utf-8'))
    out = {}
    for url in urls:
        try:
            h = get(url)
        ...
        time.sleep(0.15)
    json.dump(out, open(os.path.join(base, 'carrier_official.json'), 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
    print('完成', len(out))
